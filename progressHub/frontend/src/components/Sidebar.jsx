...
const navItems = [
  ...
  { label: "Tasks", path: "/student/tasks", icon: (
    <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
  ...
  { label: "Reminders", path: "/student/reminders", icon: (
    <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
...
const comingSoon = ["/student/homework", "/student/attendance", "/student/quizzes", "/student/reminders"];
  ...
  return (
    <>
      <style>{`
        ...
        .ss-navlink:hover { background: rgba(245,158,11,0.10); color: #ffffff; transform: translateX(4px); }
        .ss-navlink.active {
          background: linear-gradient(90deg, rgba(245,158,11,0.22), rgba(245,158,11,0.05));
          color: #f59e0b; font-weight: 600;
        }
        .ss-navlink.active::before {
        ...
        .ss-disabled {
          display: flex; align-items: center; gap: 12px;
          padding: 11px 14px; border-radius: 10px;
          color: #5a7a96; font-size: 14px; font-weight: 500;
          cursor: not-allowed; opacity: 0.65;
        }
        .ss-soon {
          font-size: 10px; font-weight: 700; color: #f59e0b;
          background: rgba(245,158,11,0.15); padding: 2px 7px;
        ...
        .ss-logout:hover { background: rgba(248,113,113,0.15); transform: translateX(4px); }
        .ss-close {
          display: none; background: none; border: none;
          cursor: pointer; color: #b8d4ea; padding: 6px;
          border-radius: 8px; margin-left: auto; transition: all 0.2s ease;
        }
        .ss-close:hover { color: #fff; background: rgba(255,255,255,0.1); }
        @media (max-width: 768px) {
          .ss-close { display: flex; align-items: center; }
        }
      `}</style>
      ...
      <aside style={{
        ...
        <div style={styles.top}>
          <div style={styles.closeRow}>
            ...
            <button className="ss-close" onClick={onClose} aria-label="Close menu">
          ...
          <nav style={styles.nav}>
            {navItems.map((item) => (
              comingSoon.includes(item.path) ? (
                <div key={item.path} className="ss-disabled" title="Coming in next module">
                  <span style={styles.icon}>{item.icon}</span>
                  <span style={styles.label}>{item.label}</span>
                  <span className="ss-soon">Soon</span>
                </div>
              ) : (
                <NavLink
                  key={item.path}
                  to={item.path}
                  className={({ isActive }) => "ss-navlink" + (isActive ? " active" : "")}
                  onClick={onClose}
                >
                  <span style={styles.icon}>{item.icon}</span>
                  <span style={styles.label}>{item.label}</span>
