...
import { Calendar, CheckCircle2, AlertTriangle, XCircle, Info, RefreshCw, UserCheck } from 'lucide-react';
...
export default function AttendanceModule({ currentUser }: AttendanceModuleProps) {
  ...
  const fetchClassStudentsAndRecords = async () => {
    try {
      setLoading(true);
      // Fetch only students belonging to the chosen classroom ID
      const stdList = await dbService.getAllStudents(selectedClassId);
      setStudents(stdList);

      // Initialize sheets with present
  ...
  const handleSubmitAttendance = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Send helpful notifications to students
            ...
            <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 pb-3 mb-4 border-b border-slate-50">
              ...
              <div className="flex items-center gap-2 shrink-0">
                ...
                <select
                  ...
                  className="px-3 py-1.5 border border-slate-200 rounded-xl text-xs bg-slate-500 bg-opacity-5 hover:bg-opacity-10 focus:outline-none focus:ring-2 focus:ring-emerald-500 font-bold"
                >
            ...
            <form onSubmit={handleSubmitAttendance} className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <select
                    ...
                    className="w-full px-3 py-2 border border-slate-200 rounded-lg text-slate-755 bg-slate-50 focus:outline-none focus:ring-2 focus:ring-emerald-500 text-sm"
                  >
                  ...
                  <table className="w-full text-left border-collapse">
                    <thead>
                      <tr className="bg-slate-50 text-slate-500 font-bold text-xs border-b border-slate-100 uppercase tracking-wider">
                        ...
                        <th className="py-3 px-4 text-center">Late (75%)</th>
                        ...
                        return (
                          <tr key={student.id} className="hover:bg-slate-50/50 transition-colors">
                            <td className="py-3.5 px-4 font-bold text-slate-800">
                              {student.full_name}
                              {student.roll_no && <span className="text-[10px] bg-slate-100 text-slate-600 px-2 py-0.5 rounded ml-2 font-mono">{student.roll_no}</span>}
                            </td>
                              ...
                              <label className="inline-flex items-center justify-center cursor-pointer">
                                ...
                                <span className={`p-2 rounded-full border transition-all ${
                                  currentStatus === 'present'
                                    ...
                                    : 'border-slate-205 text-slate-300 hover:border-slate-305'
                                }`}>
              ...
              <div className="flex justify-end pt-2">
                <button
                  ...
                  className="px-6 py-2.5 bg-emerald-600 text-white rounded-lg font-semibold text-sm shadow-sm hover:bg-emerald-700 transition disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer"
                >
                  {submitting ? 'Submitting Registry...' : 'Save Attendance Sheet'}
                </button>
              </div>
            </form>
          </div>
              ...
              <div className="space-y-3 max-h-[460px] overflow-y-auto pr-1">
                {attendanceRecords.slice(0, 10).map((rec) => (
                  <div key={rec.id} className="bg-slate-50/50 p-3 rounded-xl border border-slate-100/80 flex items-center justify-between text-xs">
                    <div>
                      <p className="font-semibold text-slate-800">{rec.student_name}</p>
